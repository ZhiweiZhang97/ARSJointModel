...
from evaluation.evaluation_model import evaluation_joint
    ...
    for epoch in range(args.epochs + 5):
        ...
        for i, batch in enumerate(t):
            ...
            if epoch < args.epochs // 2:
                _, _, _, rationale_loss, _ = model(encoded_dict, transformation_indices,
                                                   rationale_label=padded_label.to(device),
                                                   train=True)
                loss = rationale_loss
                loss.backward()
                if (i + 1) % (args.batch_size_accumulated // args.batch_size_gpu) == 0:
                    optimizer.step()
                    optimizer.zero_grad()
                    t.set_description(f'Epoch {epoch}, iter {i}, rationale loss: {round(loss.item(), 4)}')
            elif epoch < args.epochs:
                _, _, abstract_loss, _, _ = model(encoded_dict, transformation_indices,
                                                  abstract_label=batch['abstract_label'].to(device),
                                                  train=True)
                loss = abstract_loss
                loss.backward()
                if (i + 1) % (args.batch_size_accumulated // args.batch_size_gpu) == 0:
                    optimizer.step()
                    optimizer.zero_grad()
                    t.set_description(f'Epoch {epoch}, iter {i}, abstract loss: {round(loss.item(), 4)}')
            else:
        ...
        if epoch < 10:
            train_score = evaluation_joint(model, train_set, args, tokenizer)
            print(f'Epoch {epoch} train abstract score:', train_score[1])
            dev_score = evaluation_joint(model, dev_set, args, tokenizer)
            print(f'Epoch {epoch} dev abstract score:', dev_score[1])
        elif epoch < 20:
            train_score = evaluation_joint(model, train_set, args, tokenizer)
            print(f'Epoch {epoch} train rationale score:', train_score[0])
            dev_score = evaluation_joint(model, dev_set, args, tokenizer)
            print(f'Epoch {epoch} dev rationale score:', dev_score[0])
        else:
            train_score = evaluation_joint(model, train_set, args, tokenizer)
            print(f'Epoch {epoch} train abstract score:', train_score[0],
                  f'Epoch {epoch} train rationale score:', train_score[1])

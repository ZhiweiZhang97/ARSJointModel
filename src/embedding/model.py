...
class AbstractAttention(nn.Module):
    ...
    def forward(self, sentence_reps, token_mask, claim_reps):
        # att_s = self.dropout(x.view(-1, x.size(-1)))
...
class JointModelClassifier(nn.Module):
    ...
    def forward(self, encoded_dict, transformation_indices, abstract_label=None, rationale_label=None,
        ...
        bert_token = bert_out[batch_indices, indices_by_batch, :]

        sentence_representations, sentence_mask, word_att_score = self.word_attention(bert_token, mask)

        title_abstract_token = range(1, batch_indices.shape[1])
        sentence_token = range(2, batch_indices.shape[1])
        sentence_mask = sentence_mask[:, sentence_token]
        title_abstract_mask = mask[:, title_abstract_token, :]
        claim_representations = sentence_representations[:, 0, :]
        title_abstract_representations = sentence_representations[:, title_abstract_token, :]
        sentence_representations = sentence_representations[:, sentence_token, :]

        abstract_retrieval, abstract_sentence_att_score = self.abstract_retrieval(title_abstract_representations,
                                                                                  title_abstract_mask,
        ...
        claim_sentence = torch.mul(claim_representations.unsqueeze(1), sentence_representations)
        ...
        paragraph_representations, paragraph_sentence_att_score = self.sentence_attention(sentence_representations,
                                                                                          sentence_mask, valid_scores)

        claim_paragraph = torch.mul(claim_representations, paragraph_representations)
        ...
        if rationale_label is not None:
            ...
            sentence_loss1 = self.retrieval_rationale_criterion(abstract_sentence_att_score.view(-1),
                                                                rationale_pred_label.view(-1).detach())
            sentence_loss2 = self.retrieval_rationale_criterion(rationale_pred_label.view(-1),
                                                                abstract_sentence_att_score.view(-1).detach())
